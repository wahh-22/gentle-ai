...
| CON-08 | SDD runtime attempt ledger (change-level `Complete`) + runtime-compact objective (work-unit-scoped) | Consumer / persisted artifact | `internal/sddstatus/runtime_ledger.go` **and** `internal/sddstatus/runtime_compact.go` (2) | SDD, **conditionally**: only if SDD's own store gains durable, cumulative CAS-like properties; otherwise `AuthorityStore` (native authority) owns attempts | `sdd-continue`, `sdd-status`, apply/verify phases | `internal/sddstatus/runtime_ledger.go`, `internal/sddstatus/runtime_compact.go`; dated evidence — issue #2133 (change-level `Complete` contradicting work-unit-scoped objectives), fixed by PR #2151 merged 2026-08-02 (postdates the `ece470da` pin; recorded as dated evidence, not part of the pinned snapshot state) | Per decision 9's adopted default — not an unconditional single owner |
| CON-09 | OpenCode adapter dispatch + reviewer-result plugin | Consumer / adapter | `internal/agents/opencode` + `internal/assets/opencode/plugins` (1 in-repo dispatch surface; behavioral conformance not traced this pass) | `ReviewAdapter` | OpenCode host runtime | `internal/agents/opencode/adapter.go`, `internal/assets/opencode/plugins/review-result-artifacts.ts` | KEEP — structurally already a thin adapter; see Inventory gaps for unverified behavioral scope |
| CON-10 | Pi adapter dispatch | Consumer / adapter | `internal/agents/pi` (1 in-repo dispatch surface; behavioral conformance not traced this pass) | `ReviewAdapter` | Pi host runtime | `internal/agents/pi/adapter.go` | KEEP |
| CON-11 | Claude adapter dispatch | Consumer / adapter | `internal/agents/claude` (1 in-repo dispatch surface; behavioral conformance not traced this pass) | `ReviewAdapter` | Claude host runtime | `internal/agents/claude/adapter.go` | KEEP |
...
- **CON-09/CON-10/CON-11 behavioral depth.** Adapter files were enumerated by path and confirmed to exist; whether each currently constructs flags, revisions, or recovery bindings (which the target `ReviewAdapter` boundary forbids) was not traced line-by-line in this pass. A future wave should verify this before Wave 4 (Thin consumers) begins.
...
CON-08's target owner cell states the conditional default verbatim, per the requirement that this row must not assert an unconditional single owner: SDD owns attempts only if its own store gains durable, cumulative, CAS-like properties; until then, native authority (`AuthorityStore`) owns attempts.
