...
| ID | Surface | Kind | Current owner(s) | Target owner | Consumers | Evidence | Target disposition |
|---|---|---|---|---|---|---|---|
| ART-01 | Mutable CAS authority record | Active artifact | `internal/reviewtransaction` (1) | `AuthorityStore` | `ReviewCore`, all five review-context hooks | `internal/reviewtransaction/store.go`, `internal/reviewtransaction/compact_store.go` | KEEP — one of the two active review-lifecycle artifacts; never delivery authority |
| ART-02 | Immutable terminal receipt | Active artifact | `internal/reviewtransaction` (1) | `AuthorityStore` | All five review-context hooks, SDD informational context | `internal/reviewtransaction/receipt.go`, `internal/reviewtransaction/rar_native_receipt.go` | KEEP — the other active review-lifecycle artifact; never delivery authority |
...
## Review-context hooks and consumers (`CON`)

| ID | Surface | Kind | Current owner(s) | Target owner | Consumers | Evidence | Target disposition |
|---|---|---|---|---|---|---|---|
| CON-01 | `post-apply` review-context hook | Review-context hook | `internal/reviewtransaction` (1) | `ReviewContext` | Post-apply lifecycle hook | `internal/reviewtransaction/receipt.go:133`; ordering at `internal/cli/review_operation_contract.go:1457-1460` | KEEP — reports review context only; ordinary policy owns delivery |
| CON-02 | `pre-commit` review-context hook | Review-context hook | `internal/reviewtransaction` (1) | `ReviewContext` | Pre-commit hook | `internal/reviewtransaction/receipt.go:134`; ordering as above | KEEP — reports review context only; ordinary policy owns delivery |
| CON-03 | `pre-push` review-context hook | Review-context hook | `internal/reviewtransaction` (1) | `ReviewContext` | Pre-push hook | `internal/reviewtransaction/receipt.go:135`; ordering as above | KEEP — reports review context only; ordinary policy owns delivery |
| CON-04 | `pre-pr` review-context hook | Review-context hook | `internal/reviewtransaction` (1) | `ReviewContext` | Pre-PR hook | `internal/reviewtransaction/receipt.go:136`; gate-specific base-relationship logic at `:289,304,316,348,355`; ordering as above | KEEP — reports review context only; ordinary policy owns delivery |
| CON-05 | `release` review-context hook | Review-context hook | `internal/reviewtransaction` (1) | `ReviewContext` | Release hook | `internal/reviewtransaction/receipt.go:137`; gate-specific logic at `:304,307,316`; ordering as above | KEEP — reports review context only; ordinary policy owns delivery |
| CON-06 | SDD review-context observation | Consumer | `internal/sddstatus` (1) | `SDD` | `sdd-status`, `sdd-continue` | `internal/sddstatus/review_gate.go` | MERGE — request receipt context only to report it; never independently reinterpret review state or control verification/archive |
