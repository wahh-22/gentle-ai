...
func installContentAddressedFile(path string, payload []byte) error {
	...
	if err := linkFile(tempPath, path); err != nil {
		if os.IsPermission(err) {
			if fallbackErr := writeContentAddressedFile(path, payload); fallbackErr == nil {
				return nil
			} else {
				err = fallbackErr
			}
...
func writeContentAddressedFile(path string, payload []byte) (err error) {
