...
func TestDiscoverIntendedUntrackedExcludesRegisteredNestedWorktree(t *testing.T) {
	...
	intended, err := (SnapshotBuilder{Repo: repo}).DiscoverIntendedUntracked(context.Background())
	if err != nil {
		t.Fatalf("DiscoverIntendedUntracked with a registered nested worktree: %v", err)
	}
...
func TestDiscoverIntendedUntrackedFromInsideNestedWorktreeStaysScoped(t *testing.T) {
	requireSnapshotGit(t)
	repo := initSnapshotRepo(t)
	...
	intended, err := (SnapshotBuilder{Repo: nested}).DiscoverIntendedUntracked(context.Background())
	if err != nil {
		t.Fatalf("DiscoverIntendedUntracked from inside the nested worktree: %v", err)
	}
...
func TestDiscoverIntendedUntrackedIgnoresSiblingLinkedWorktree(t *testing.T) {
	requireSnapshotGit(t)
	repo := initSnapshotRepo(t)
	...
	intended, err := (SnapshotBuilder{Repo: repo}).DiscoverIntendedUntracked(context.Background())
	if err != nil {
		t.Fatalf("DiscoverIntendedUntracked with a sibling linked worktree: %v", err)
	}
...
func TestDiscoverIntendedUntrackedRefusesForeignEmbeddedRepository(t *testing.T) {
	requireSnapshotGit(t)
	repo := initSnapshotRepo(t)
	embedded := filepath.Join(repo, "vendor", "embedded")
	gitSnapshot(t, repo, "init", "-q", embedded)

	_, err := (SnapshotBuilder{Repo: repo}).DiscoverIntendedUntracked(context.Background())
	if err == nil {
		t.Fatal("DiscoverIntendedUntracked admitted a foreign embedded repository")
	}
	message := err.Error()
